...
j2_tmpl = env.get_template('sup_devices_info.jinja2')
# 加载变量文件，解析成列表/字典
with open('sup_devices_info.yaml') as f:
    vars = yaml.safe_load(f.read())
result = j2_tmpl.render(vars)
f=open(r'../../sup_info.json','w')
print(result,file=f)
f.close()
...
file_old = open(r'../../sup_info.json','rb+')
m = 100
...
file = open(r'../../sup_info.json','a')   #再次打开文件添加 ']'方括号闭合json文件数据
file.seek(0)
file.write(']')
file_old.close()
print('登录信息生成完毕!!')
